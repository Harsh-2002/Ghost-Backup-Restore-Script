...
S3_MYSQL_FOLDER="MySQL"

# List objects in the S3 MySQL folder and get the latest backup file
LATEST_MYSQL_BACKUP=$(aws s3 ls "s3://$S3_BUCKET/$S3_MYSQL_FOLDER/" | sort | tail -n 1 | awk '{print $4}')

# Check if a MySQL backup file was found
if [ -n "$LATEST_MYSQL_BACKUP" ]; then
    # Download the latest MySQL backup from S3 and restore it
    aws s3 cp "s3://$S3_BUCKET/$S3_MYSQL_FOLDER/$LATEST_MYSQL_BACKUP" - | mysql -u $DB_USER -p$DB_NAME
...
if [ $? -eq 0 ]; then
    ...
    echo "Ownership and permissions set for specific folders and restarting all services."
   else
